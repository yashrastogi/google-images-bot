...
Future<List<Uri>> getGoogleImages(String query, {int count = 5}) async {
  var url = Uri.parse("https://www.google.com/search?tbm=isch&q=${Uri.encodeFull(query)}");
  var document = (await Dio().getUri(url)).data;
  var soup = Beautifulsoup(document);
  List<Uri> res = List();
  soup.find_all('img').forEach((elem) {
    if (elem.attributes['src'] != null) if (elem.attributes['src'].contains("gstatic.com")) {
      if (count == 0) return;
      count--;
...
void main() {
  ...
  var teledart = TeleDart(Telegram(io.Platform.environment['TELEGRAM_API_TOKEN']), Event());

  teledart.start().then((me) => print('${me.username} is initialised.'));

  teledart.onCommand('greet').listen(((message) => teledart.replyMessage(message,
  ...
  teledart.onMessage(keyword: 'dart').where((message) => message.text.contains('telegram')).listen((message) => teledart
      .replyPhoto(message, 'https://raw.githubusercontent.com/DinoLeung/TeleDart/master/example/dash_paper_plane.png',
  ...
  try {
    teledart
        ...
        .onError((handleError) => print("OnError: ${handleError}"));
  } catch (e) {
    print("Catch: ${e}");
  }
